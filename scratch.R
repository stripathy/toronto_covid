...
neighbourhood_profiles %>% filter(Characteristic == ' Average after-tax income of households in 2015 ($)') 


avg_income_vector = neighbourhood_profiles %>% filter(X_id == 1030)
...
colnames(avg_incomes_transposed) = c("norm_neighbourhood_name", 'avg_income')
